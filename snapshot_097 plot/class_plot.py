...
for file in files:
    ...
    if filename[1] == '.hdf5':
        ...
        pos_c = pos = bin["PartType0"]["Coordinates"][:]
        ...
        while (np.fabs(Mp - Mp0) > 0.001):
            ...
            # 分别取出不同趋势的粒子的id
            pos_mm = pos_c[np.where(pgrp > 1)]  # planet id
            # print(pos_mm)
            # print('-----')

            pos_disk = pos_c[np.where(pgrp < 1)]  # disk id
            # print(pos_disk)
            # print('-----')

            pos_mesccpe = pos_c[np.where(pgrp == 1)]  # escape id
            # print(pos_mesccpe)
            # print('-----')
        ...
        s1 = ax.scatter(x_slice_m, y_slice_m, s=dot_size, c='r', linewidth=line_width_thin)
        s2 = ax.scatter(x_slice_d, y_slice_d, s=dot_size, c='b', linewidth=line_width_thin)
        s3 = ax.scatter(x_slice_e, y_slice_e, s=dot_size, c='g', linewidth=line_width_thin)

        xlim((-4e9, 4e9))
        ylim((-4e9, 4e9))

        # ax.set_axis below(True)  # 网格显现在图形下方
        # grid()
        ...
        legend(handles=[s1, s2, s3], labels=["Planet", "Disk", "Escape"], loc='best', fontsize='xx-small')
