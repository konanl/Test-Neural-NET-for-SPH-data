...
int TipsyInitialize(TCTX *pctx,int bNative,char *pchInFile)
{
	...
	ctx = (TCTX)malloc(sizeof(struct TipsyContext));
	assert(ctx != NULL);
	ctx->bNative = bNative;
	if (pchInFile != NULL) {
		ctx->pchInFile = (char *)malloc(strlen(pchInFile)+1);
...
struct base_particle *pTipsyReadNative(TCTX ctx,int *piType,double *pdSoft)
{
	...
	case TIPSY_TYPE_GAS:
		if (i == 0) {
			...
			if (ctx->nBlock > 0) {
				if (ctx->gp == NULL) {
					ctx->gp = (struct gas_particle *)malloc(TIPSY_BLOCK*sizeof(struct gas_particle));
					assert(ctx->gp != NULL);
					}
	...
	case TIPSY_TYPE_DARK:
		...
		jump_dark:
			...
			if (ctx->nBlock > 0) {
				if (ctx->dp == NULL) {
					ctx->dp = (struct dark_particle *)malloc(TIPSY_BLOCK*sizeof(struct dark_particle));
					assert(ctx->dp != NULL);
					}
	...
	case TIPSY_TYPE_STAR:
		...
		jump_star:
			...
			if (ctx->nBlock > 0) {
				if (ctx->sp == NULL) {
					ctx->sp = (struct star_particle *)malloc(TIPSY_BLOCK*sizeof(struct star_particle));
					assert(ctx->sp != NULL);
					}
...
int TipsyReadAll(TCTX ctx)
{
	 ...
	 */
	assert(ctx->iCurr == 0);
	if (ctx->nGas + ctx->head.nsph > ctx->nMaxGas) {
		ctx->nMaxGas = ctx->nGas + ctx->head.nsph;
		ctx->gp = (struct gas_particle *)realloc(ctx->gp,ctx->nMaxGas*sizeof(struct gas_particle));
		if (ctx->gp == NULL) {
			fprintf(stderr,"ERROR(TispyReadAll): Could not extend gas particle array.\n");
			return 1;
			}
		}
	if (ctx->nDark + ctx->head.ndark > ctx->nMaxDark) {
		ctx->nMaxDark = ctx->nDark + ctx->head.ndark;
		ctx->dp = (struct dark_particle *)realloc(ctx->dp,ctx->nMaxDark*sizeof(struct dark_particle));
		if (ctx->dp == NULL) {
			fprintf(stderr,"ERROR(TispyReadAll): Could not extend dark particle array.\n");
			return 2;
			}
		}
	if (ctx->nStar + ctx->head.nstar > ctx->nMaxStar) {
		ctx->nMaxStar = ctx->nStar + ctx->head.nstar;
		ctx->sp = (struct star_particle *)realloc(ctx->sp,ctx->nMaxStar*sizeof(struct star_particle));
		if (ctx->sp == NULL) {
...
{
	return(ctx->head.nbodies + ctx->nGas + ctx->nDark + ctx->nStar);
	}
